...
class Bill_App:
    def __init__(self,root):
        ...
        #Customer details
        F1 = LabelFrame(self.root,text ="Customer Details",font=("times new roman",15,"bold"),fg="#FF8C00",bg = bg_color)
        ...
        c_bill_txt = Entry(F1, width=15, textvariable=self.search_bill, font=("arial", 15, "bold"), bd=7,relief=SUNKEN).grid(row=0, column=5, padx=5, pady=10)

        bill_btn = Button(F1, text="Search", width=15,command=self.find_bill, bd=7, font="arial 12 bold").grid(row=0, column=6, padx=7, pady=10)

        # ========COSMETICS FRAME=======

        F2 = LabelFrame(self.root, bd=10, relief=GROOVE, text="Mains", font=("times new roman", 15, "bold"), fg="#FF8C00",
                        bg=bg_color)
        F2.place(x=5, y=170, width=340, height=335)

        a1_lbl = Label(F2, text="Burger", font=("times new roman", 16, "bold"), bg=bg_color, fg="lightgreen").grid(
        ...
        b1_lbl = Label(F3, text="Fries", font=("times new roman", 16, "bold"), bg=bg_color, fg="lightgreen").grid(
            row=0, column=0, padx=10, pady=10, sticky="w")
        ...
        F4 = LabelFrame(self.root, bd=10, relief=GROOVE, text="Beverages", font=("times new roman", 15, "bold"), fg="#FF8C00",bg=bg_color)
        F4.place(x=670, y=170, width=340, height=335)

        c1_lbl = Label(F4, text="Coke", font=("times new roman", 16, "bold"), bg=bg_color, fg="lightgreen").grid(
            row=0, column=0, padx=10, pady=10, sticky="w")
        ...
        # Button frame
        F6 = LabelFrame(self.root, bd=10, relief=GROOVE, text="Bill Menu",font=("times new roman", 15, "bold"), fg ="#FF8C00", bg=bg_color)
        ...
        s3 = Label(F6, text="Bevarages Tax", bg=bg_color, fg="white", font=("times new roman", 14, "bold")).grid(
        ...
        total_btn = Button(btn_F, text="Total", bg="#17202A", fg="white", bd=2, pady=15, width=10,command=self.total,
                           font="arial 15 bold").grid(row=0, column=0, padx=5, pady=5)
        GBill_btn = Button(btn_F, text="Generate Bill", bg="#17202A", fg="white", bd=2, pady=15, width=10,command=self.bill_area,
                           font="arial 15 bold").grid(row=0, column=1, padx=5, pady=5)
        Clear_btn = Button(btn_F, text="Clear", bg="#17202A", fg="white", bd=2, pady=15, width=10,command=self.clear_data,
                           font="arial 15 bold").grid(row=0, column=2, padx=5, pady=5)
        Exit_btn = Button(btn_F, text="Exit", bg="#17202A", fg="white", bd=2, pady=15, width=10,command=self.Exit_app,
    ...
    def total(self):
        ...
        self.total_bevarages_price = float(
            (self.c_c1) +
            (self.c_c2) +
            (self.c_c3) +
            (self.c_c4) +
        ...
        self.c_price.set("Rs. " + str(self.total_bevarages_price))
        self.z_tax= round((self.total_bevarages_price * 0.05), 2)
        self.c_tax.set("Rs. " + str(self.z_tax))

        self.Total_bill=float( self.total_mains_price+
                               self.total_sides_price+
                               self.total_bevarages_price+
                               self.x_tax+
                               self.y_tax+
                               self.z_tax
                             )
    ...
    def bill_area(self):
        if self.c_name.get()=="" or self.c_phone.get()=="":
            ...
            if self.a3_txtt.get() != 0:
                self.txtarea.insert(END, f"\n Shawarma\t\t{self.a3_txtt.get()}\t\t{self.a_a3}")
            ...
            if self.b2_txtt.get() != 0:
                self.txtarea.insert(END, f"\n Nugge\t\t{self.b2_txtt.get()}\t\t{self.b_b2}")
            if self.b3_txtt.get() != 0:
                self.txtarea.insert(END, f"\n B3\t\t{self.b3_txtt.get()}\t\t{self.b_b3}")
            if self.b4_txtt.get() != 0:
                self.txtarea.insert(END, f"\n B4\t\t{self.b4_txtt.get()}\t\t{self.b_b4}")
            if self.b5_txtt.get() != 0:
                self.txtarea.insert(END, f"\n B5\t\t{self.b5_txtt.get()}\t\t{self.b_b5}")

            # ===Drinks======
            if self.c1_txtt.get() != 0:
                self.txtarea.insert(END, f"\n C1\t\t{self.c1_txtt.get()}\t\t{self.c_c1}")
            if self.c2_txtt.get() != 0:
                self.txtarea.insert(END, f"\n C2\t\t{self.c2_txtt.get()}\t\t{self.c_c2}")
            if self.c3_txtt.get() != 0:
                self.txtarea.insert(END, f"\n C3\t\t{self.c3_txtt.get()}\t\t{self.c_c3}")
            if self.c4_txtt.get() != 0:
                self.txtarea.insert(END, f"\n C4\t\t{self.c4_txtt.get()}\t\t{self.c_c4}")
            if self.c5_txtt.get() != 0:
                self.txtarea.insert(END, f"\n C5\t\t{self.c5_txtt.get()}\t\t{self.c_c5}")

            self.txtarea.insert(END, f"\n-------------------------------------")
            if self.a_tax.get()!="Rs. 0.0":
                self.txtarea.insert(END, f"\n A Tax\t\t\t{self.a_tax.get()}")
            if self.b_tax.get()!="Rs. 0.0":
                self.txtarea.insert(END, f"\n B Tax\t\t\t{self.b_tax.get()}")
            if self.c_tax.get()!="Rs. 0.0":
                self.txtarea.insert(END, f"\n C Tax\t\t\t{self.c_tax.get()}")
    ...
    def save_bill(self):
        ...
        if op > 0:
            ...
            f1 = open("C:\\Users\\ritik\PycharmProjects\\tkinter_project\\" + str(self.bill_no.get()) + ".txt", "w")
        ...
        c.execute(
            "CREATE TABLE qqq(c_phone integer, c_name text,bill_no);")

        c.execute("INSERT INTO qqq(c_phone,c_name,bill_no) VALUES(?,?,?)",(str(self.c_phone.get()),str(self.c_name.get()),str(self.bill_no.get())))
        #c.execute('SELECT * FROM sale2')
        conn.commit()
        print(str(self.c_phone.get()))
        print("data entered")
        conn.close()

    def find_bill(self):
        for i in os.listdir("Invoice/"):
            if i.split('.')[0] == self.search_bill.get():
                f1 = open(f"C:\\Users\\ritik\\PycharmProjects\\tkinter_project\\{i}", "r")
